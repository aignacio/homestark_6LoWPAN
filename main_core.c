...
#define REPLY_TIMEOUT (0.5 * CLOCK_SECOND)
...
static char pub_topic[26] = "0000000000000000/anderson\0";
...
PROCESS_THREAD(example_mqttsn_process, ev, data)
{
  ...
  mqtt_sn_set_debug(1);
  uip_ip6addr(&broker_addr, 0xaaaa, 0, 0, 0, 0, 0, 0, 0x1);
  ...
  // Copiando device_id também para o client id
  sprintf(mqtt_client_id,"%02X%02X%02X%02X%02X%02X%02X%02X",linkaddr_node_addr.u8[0],
          linkaddr_node_addr.u8[1],linkaddr_node_addr.u8[2],linkaddr_node_addr.u8[3],
          linkaddr_node_addr.u8[4],linkaddr_node_addr.u8[5],linkaddr_node_addr.u8[6],
          linkaddr_node_addr.u8[7]);
  ...
  mqtt_sn_send_connect(&mqtt_sn_c,(char *)mqtt_client_id,mqtt_keep_alive);
